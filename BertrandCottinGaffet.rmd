...
fig_signif_coefs_001 <- get_fig_signif_coefs(mods_summary, 0.001) +
  scale_y_continuous(limits = c(-25, 25))
ggarrange(fig_coefs, fig_signif_coefs_001, ncol = 2, common.legend = TRUE, 
          legend = "right")
```
...
```{r, fig_presence, fig.height = 6, fig.cap = "Diagrammes en bâtons de la présence ou de l'absence des motifs génomiques significatifs en fonction de la valeur absolue du coefficient ajusté par régression logistique. En haut, pour les variables sélectionnées par Lasso, en bas par stability selection avec un seuil de stabilité à 0.9. Les coefficients colorés en vert sont négatifs (sensibilité) et en rouges, positifs (résistance)", eval = F}
fig_pres_lasso <- get_fig_prensence(X.train, mods_summary, "Lasso", 0.001)
fig_pres_stab <- get_fig_prensence(X.train, mods_summary, 
                                   "StabSel-0.9 (cv_1sd)", 0.001)

fig <- ggarrange(fig_pres_lasso + rremove("ylab") + rremove("xlab"), 
                 fig_pres_stab + rremove("ylab") + rremove("xlab"),
                 nrow = 2, common.legend = TRUE, legend = "right")
annotate_figure(fig, left = textGrob("n souches", rot = 90, vjust = 1),
                bottom = textGrob("Valeur absolue du coefficient du motif génomique"))
```
