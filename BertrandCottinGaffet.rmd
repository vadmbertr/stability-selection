...
\quad Tout d'abord nous pouvons observer que pour les $\lambda$ d'index 25 à 100, la probabilité de sélection des variables les plus "importantes" varie peu.
...
\quad Nous constatons que les coefficients obtenus par stability selection (quelque soit le seuil de stabilité) sont majoritairement positifs tandis que les coefficients du Lasso classique sont positifs et négatifs.
...
\quad La capacité des modèles de stability selection à capter la résistance plutôt que la sensibilité et à conserver des motifs plus communs expliqueraient donc les bons résultats prédictifs donnés par celle-ci malgré des supports nettement plus compacts.
A noter que dans l'optique d'expliquer la résistance aux antibiotiques plus que de la prédire il n'est pas nécessairement intéressant de limiter l'amplitude des coefficients, en revanche le nombre de variables à considérer et la stabilité de la sélection reste un critère important.
...
être dûe au fait que la procédure Lasso classique conserve le bruit de nombreux motifs génomiques rares sensibles en plus des motifs résistants plus récurrents.
