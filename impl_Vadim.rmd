...
ggplot(data = mods_res) +
        geom_jitter(aes(x = nzero, y = score, colour = model, shape = model), width = .15, height = .15) +
        scale_shape_manual(values = 1:nrow(mods_res)) +
        theme_bw()
```

```{r}
stab_sel_1sd_roc <- get_roc(X.train, y.train, X.test, y.test, vars_idx_1sd)
stab_sel_min_roc <- get_roc(X.train, y.train, X.test, y.test, vars_idx_min)
stab_sel_cv_roc <- list(sd = stab_sel_1sd_roc, min = stab_sel_min_roc)
ggroc(stab_sel_cv_roc) +
        ggtitle(paste0("Courbes ROC - sd : AUC = ", round(stab_sel_cv_roc$sd$auc, digits = 2),
                       " - min : AUC = ", round(stab_sel_cv_roc$min$auc, digits = 2))) +
        theme_bw()
```
