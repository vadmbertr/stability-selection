...
run_stability_selection_model <- function(X_train, y_train, X_test, y_test, stability_path) {
  ...
  ranges <- sapply(vars_idx, function(v_idx) get_coef_range(X_train, y_train, v_idx))
...
reg_path <- merge(reg_path, max_freq, by = "var")
stab_path <- merge(stab_path, max_freq, by = "var")

# cast en factor avec un ordre de levels precis pour que les chemins des graphs soient bien superposes
...
ggplot(data = mods_summary) +
        ...
        scale_shape_manual(name = "Modèle", values = 1:nrow(mods_summary)) +
        scale_color_discrete(name = "Modèle") +
        labs(x = "Amplitude des coefficients", y = "Taux d'agrément") +
        theme_bw()
```
...
fig_stab <- ggplot(data = stab_path) +
        ...
        labs(title = "Chemin de stabilité", x = "Lambda index", y = "Fréquence de sélection")
